...
def search():
    if request.method == 'POST':
        ...
        return redirect(url_for('get_summoner_info', summoner_name=summoner_name))
...
def get_summoner_info(summoner_name):
    ...
    if response.status_code == 200:
        ...
        profile = summoner_data['profileIconId']
        profile_url = f'https://ddragon.leagueoflegends.com/cdn/13.16.1/img/profileicon/{profile}.png'
        return render_template('result.html', profile_url=profile_url, summoner_data=summoner_data)
    elif response.status_code == 404:
        return jsonify({'error': 'Summoner not found'}), 404
    else:
        return jsonify({'error': 'An error occurred'}), 500
...
def get_summoner_info(summoner_name):
    ...
    if response.status_code == 200:
        ...
        profile = summoner_data['profileIconId']
        profile_url = f'https://ddragon.leagueoflegends.com/cdn/13.16.1/img/profileicon/{profile}.png'
        return jsonify(profile_url=profile_url, summoner_data=summoner_data)
    elif response.status_code == 404:
        return jsonify({'error': 'Summoner not found'}), 404
    else:
        return jsonify({'error': 'An error occurred'}), 500
...
def search():
    if request.method == 'POST':
        summoner_name = request.form['summoner_name']
        return redirect(url_for('result', summoner_name=summoner_name))
        #return redirect(url_for('get_summoner_info', summoner_name=summoner_name))
    return render_template('search.html')
